...
dependency "secondary" {
  config_path = "../../prddr/mssql"
}

terraform {
  source = "../../../../modules/mssqlfgmodule"
...
inputs = {
  ...
  fg_name = "${local.env_config.global.name_prefix[0]}${local.environment.mssqlserver.failover_group.name}"
  resource_group_location = local.env_config.global.locations[0]
  database_db1_name = local.environment.mssqlserver.dbs.db1.name
  database_db2_name = local.environment.mssqlserver.dbs.db2.name
  primary_instance_id = dependency.primary.outputs.id
  primary_instance_name = dependency.primary.outputs.database_server_name
  secondary_instance_id = dependency.secondary.outputs.id
  secondary_instance_name = dependency.secondary.outputs.database_server_name
}
