...
inputs = {
  ...
  name = "${local.env_config.global.name_prefix[0]}${local.environment.name[0]}${local.environment.aks.name[0]}"
  vm_size = local.env_config.prod_workloads.prd.aks.aks_agent_vm_size
  resource_group_location = local.env_config.global.locations[1]
  agent_count = local.env_config.prod_workloads.prd.aks.agent_count
  acr_resource_id            = dependency.acr.outputs.id
  acr_name = "${local.env_config.global.name_prefix[0]}${local.environment.name[0]}${local.environment.acr.name[0]}"
  kubernetes_version = local.environment.aks.kubernetes_version
}
